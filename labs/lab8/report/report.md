...
Для задания проблемы используется функция `ODEProblem`, а для решения -- численный метод Tsit5(), с помощью `plot` построим график решения для первого случая.
...
![График изменения оборотных средств для первого случая. julia](image/2.PNG){#fig:002 width=70%}
![Приближенный график изменения оборотных средств для первого случая. julia](image/3.PNG){#fig:003 width=70%}
![График изменения оборотных средств для первого случая. julia](image/4.PNG){#fig:004 width=70%}
![График изменения оборотных средств для первого случая. julia](image/5.PNG){#fig:005 width=70%}
![Приближенный график изменения оборотных средств для первого случая. julia](image/6.PNG){#fig:006 width=70%}
...
Для задания проблемы используется функция `ODEProblem`, а для решения -- численный метод Tsit5(), с помощью `plot` построим график решения для второго случая.
